...
#    '++++' => [:pass, ['latexmath'].to_set],
  ...
  REGEXP = {
    ...
    # (((Tigers,Big cats)))
    # ORM: also need to ignore quotes, e.g. ((("Tigers","Big cats")))
    # old version... :indexterm_macro  => /\\?(?:indexterm:(?:\[((?:\\\]|[^\]])*?)\])|\(\(\((.*?)\)\)\)(?!\)))/m,
    :indexterm_macro  => /\\?(?:indexterm:(?:\["?((?:\\\]|[^\]])*?)(?:"?)\])|\(\(\((?:"?)(.*?)(?:"?)\)\)\)(?!\)))/m,

    # indexterm2:[Tigers]
    # ((Tigers))
    # ORM: also need handling for terms in quotes, e.g. (("Tigers"))
    ...
    # ORM (work in progress): latexmath inline macro
    # latexmath:[$latex$]
    # :pass_latexmath  => /latexmath:([a-z,]*)\[((?:\\\]|[^\]])*?)\]/m,
  ...
  require 'orm_asciidoctor/list_item'
